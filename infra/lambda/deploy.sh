...
: "${RESQD_ORIGIN_SECRET:?RESQD_ORIGIN_SECRET must be set (shared secret with the CF Worker that injects x-origin-secret; generate with: openssl rand -base64 32. Also set it as the Worker secret: wrangler secret put ORIGIN_SECRET)}"
: "${RESQD_ADMIN_EMAILS:?RESQD_ADMIN_EMAILS must be set (comma-separated admin email allowlist; at least one). Example: khiebel@gmail.com}"
...
ENV_VARS="Variables={RESQD_S3_BUCKET=resqd-vault-64553a1a,RESQD_CHAIN_ENABLED=true,RESQD_CHAIN_RPC_URL=https://sepolia.base.org,RESQD_CHAIN_CONTRACT=0xd45453477aa729C157E4840e81F81D4437Ec99f3,RESQD_CHAIN_SIGNER_KEY=$RESQD_CHAIN_SIGNER_KEY,RUST_LOG=info,RESQD_AUTH_ENABLED=true,RESQD_WEBAUTHN_RP_ID=$WEBAUTHN_RP_ID,RESQD_WEBAUTHN_RP_NAME=RESQD,RESQD_WEBAUTHN_ORIGIN=$WEBAUTHN_ORIGIN,RESQD_JWT_SECRET=$RESQD_JWT_SECRET,RESQD_ORIGIN_SECRET=$RESQD_ORIGIN_SECRET,RESQD_ADMIN_EMAILS=$RESQD_ADMIN_EMAILS,RESQD_USERS_TABLE=resqd-users,RESQD_CHALLENGES_TABLE=resqd-auth-challenges,RESQD_RINGS_TABLE=resqd-rings,RESQD_COOKIE_SECURE=true,RESQD_COOKIE_SAMESITE=None,RESQD_CORS_ORIGINS=$CORS_ORIGINS,RESQD_BLOCKED_COUNTRIES=$BLOCKED_COUNTRIES}"
